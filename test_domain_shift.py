...
from testing_utils import ResultsData, load_model, test_domain_shift

parser = argparse.ArgumentParser(
    description="Testing on domain-shift ImageNet datasets."
)
parser.add_argument(
    "--data_dir", default="/media/generalstorage3/lapstorage/datasets/imagenet-r", type=str, help="Path to dataset root directory"
)
parser.add_argument(
    "--out_dir", default='/media/generalstorage3/lapstorage/out', type=str,
    help="Output root directory where CSV file with results will be created"
)
parser.add_argument(
...
parser.add_argument(
    "--imagenet", default="imagenet-c", type=str, help="Name of ImageNet dataset",
    choices=['c', 'cartoon', 'drawing', 'r', 'sketch', 'stylized16']
)
parser.add_argument(
...
def main():
    ...
    output_file = os.path.join(ARGS.out_dir, f'{exp_name}-dom-shift-{ARGS.imagenet}.csv')
    results = ResultsData.from_csv(output_file, 'domain_shift')
    ...
    for seed, ckpt_path in subdirs:
        ...
        top1_acc = test_domain_shift(ARGS, model, subcort_kwargs, ARGS.imagenet=='stylized16')
        results.update(
            exp_name=exp_name,
            seed=seed,
            dataset=f'imagenet-{ARGS.imagenet}', 
            top1_acc=top1_acc,
