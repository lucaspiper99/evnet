"""
Adapted from
https://github.com/rgeirhos/texture-vs-shape/blob/master/code/probabilities_to_decision.py and
https://github.com/rgeirhos/texture-vs-shape/blob/master/code//human_categories.py
...
class ImageNetProbabilitiesTo16ClassesMapping(ProbabilitiesToDecisionMapping):
    ...
    def probabilities_to_decision(self, probabilities):
        ...
        Keyword arguments:
        probabilities -- a ndarray with size (N, 1000)
                         (softmax output: all values should be
                         within [0,1])
        Returns:
        16-class probabilities -- ndarray with size (N, 16)
        """

        self.check_input(probabilities)
        assert probabilities.shape[-1] == 1000
        ...
        for category in get_human_object_recognition_categories():
            ...
            aggregated_class_probs.append(aggregated_value)
        aggregated_class_probs = np.stack(aggregated_class_probs, axis=1)
        return np.argmax(aggregated_class_probs, axis=-1)
